...
export function decodeTiffTexture(bytes, textureName) {
  ...
  */
  ...
  // `sourceFormat` is what the .BIN preview path keys its cutout handling off; TIFF carries
  // real alpha, so it must never be treated as a colour-keyed RAW.
  return { name: textureName, width, height, rgba, hasAlpha: samplesPerPixel === 2, sourceFormat: "TIF" };
}
